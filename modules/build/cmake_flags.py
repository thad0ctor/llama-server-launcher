...
FLAGS: list[CMakeFlag] = [
    ...
    # ── CUDA core ──
    CMakeFlag("GGML_CUDA", "Enable CUDA", "CUDA", BOOL, False, help="Master toggle for NVIDIA CUDA backend."),
    CMakeFlag(
        ...
        help=(
            "Standard cmake variable. Semi-colon list of <code>[-real|-virtual]. "
    ...
    CMakeFlag("GGML_OPENCL", "OpenCL", "Other GPU Backends", BOOL, False, backends=(BACKEND_LLAMA,)),
    CMakeFlag("GGML_KOMPUTE", "Kompute (Vulkan)", "Other GPU Backends", BOOL, False, backends=(BACKEND_IK,)),
    CMakeFlag(
        "GGML_MUSA",
        "MUSA (Moore Threads)",
        "Other GPU Backends",
    ...
    CMakeFlag("GGML_VULKAN_DEBUG", "Vulkan debug output", "Vulkan options", BOOL, False, visible_when=_vulkan_on),
    CMakeFlag(
        "GGML_VULKAN_MEMORY_DEBUG", "Vulkan memory debug", "Vulkan options", BOOL, False, visible_when=_vulkan_on
    ),
    CMakeFlag(
        "GGML_VULKAN_SHADER_DEBUG_INFO", "Shader debug info", "Vulkan options", BOOL, False, visible_when=_vulkan_on
    ),
    CMakeFlag(
    ...
    CMakeFlag("GGML_VULKAN_RUN_TESTS", "Run Vulkan tests", "Vulkan options", BOOL, False, visible_when=_vulkan_on),
    CMakeFlag(
    ...
    CMakeFlag(
        "GGML_SYCL_GRAPH", "SYCL graphs", "SYCL options", BOOL, True, backends=(BACKEND_LLAMA,), visible_when=_sycl_on
    ),
    CMakeFlag(
    ...
    # ── Metal sub-options ──
    CMakeFlag("GGML_METAL_NDEBUG", "Disable Metal debugging", "Metal options", BOOL, False, visible_when=_metal_on),
    CMakeFlag(
        "GGML_METAL_SHADER_DEBUG", "Shader debug (-fno-fast-math)", "Metal options", BOOL, False, visible_when=_metal_on
    ),
    CMakeFlag(
    ...
    CMakeFlag("GGML_AVX_VNNI", "AVX-VNNI", "CPU SIMD (x86)", BOOL, False, backends=(BACKEND_LLAMA,)),
    ...
    CMakeFlag("GGML_BMI2", "BMI2", "CPU SIMD (x86)", BOOL, True, backends=(BACKEND_LLAMA,)),
    CMakeFlag("GGML_AMX_TILE", "AMX-TILE", "CPU SIMD (x86)", BOOL, False, backends=(BACKEND_LLAMA,)),
    CMakeFlag("GGML_AMX_INT8", "AMX-INT8", "CPU SIMD (x86)", BOOL, False, backends=(BACKEND_LLAMA,)),
    CMakeFlag("GGML_AMX_BF16", "AMX-BF16", "CPU SIMD (x86)", BOOL, False, backends=(BACKEND_LLAMA,)),
    # ── BLAS / Accelerated math ──
    CMakeFlag(
        "GGML_BLAS",
        "Use BLAS",
    ...
    # ── Optimization ──
    CMakeFlag(
        "GGML_LTO", "Link-time optimization", "Optimization", BOOL, True, help="LTO — usually a small but free win."
    ),
    CMakeFlag(
        "GGML_CCACHE",
        "ccache",
    ...
    CMakeFlag("GGML_STATIC", "Static link", "Optimization", BOOL, False, help="Statically link the binaries."),
    # ── Runtime ──
    CMakeFlag("GGML_OPENMP", "OpenMP", "Runtime", BOOL, True, help="OpenMP parallelization on CPU paths."),
    CMakeFlag(
        "GGML_BACKEND_DL",
    ...
    CMakeFlag(
        ...
        "All CPU SIMD variants",
        "Runtime",
        BOOL,
        False,
        backends=(BACKEND_LLAMA,),
        visible_when=_backend_dl_on,
...
def build_autodetect_values(
    ...
    *,
    ...
    if cuda_available:
        ...
        if backend == BACKEND_LLAMA:
            ...
            if (
                ...
                and _cuda_version_satisfies(compression_flag.cuda_version_min, cuda_version)
            ):
    ...
    # ``GGML_LLAMAFILE`` is declared with ``backends=(BACKEND_LLAMA,)``
    # (~line 563), so assigning it for BACKEND_IK leaks a llama-only key
    # into the autodetect result. Callers that persist or diff the
    # returned dict against ``default_values_for_backend(BACKEND_IK)``
    # would then see a phantom ``GGML_LLAMAFILE`` entry that the IK
    # build won't ever consume.
    if backend == BACKEND_LLAMA:
        ...
        if flag.type == ENUM:
            ...
            if sv and flag.choices and sv not in flag.choices:
                errors.append(
                    (
                        flag.label,
                        f"value {sv!r} is not one of the allowed choices " f"({', '.join(flag.choices)}).",
                    )
                )
            continue
        if flag.type == BOOL:
            # BOOL flags used to slip straight through to ``_bool_str``
            # which silently coerces ANY value to ``"ON"`` / ``"OFF"``
            # via Python's truthiness rules — so a hand-edited preset
            # with ``"GGML_CUDA": "banana"`` or ``"GGML_CUDA": []``
            # produced a misleading ``-DGGML_CUDA=ON`` / ``-DGGML_CUDA=OFF``
            # instead of failing validation. Accept only real booleans
            # or canonical boolean strings; reject anything else here
            # so the user sees the bad value before cmake configure runs.
            raw_value = values[flag.key]
            ...
            errors.append(
                (
                    flag.label,
                    f"value {raw_value!r} is not a recognized boolean " f"(use true/false, on/off, 1/0, or yes/no).",
                )
            )
            continue
        if flag.validate is None:
