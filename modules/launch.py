...
class LaunchManager:
    ...
    def build_cmd(self, probe_backend=False):
        ...
        if backend == "ik_llama" and tensor_split_val:
            print(
                "WARNING: --tensor-split is not supported by current ik_llama server builds; ignoring.",
                file=sys.stderr,
            )
        else:
    ...
    def save_sh_script(self):
        ...
        try:
            with open(path, "w", encoding="utf-8") as fh:
                ...
                # ``#!/usr/bin/env bash`` (not ``#!/bin/bash``) for the
                # same reason the direct-launch fallback above uses
                # ``shutil.which("bash")`` — a Homebrew, NixOS, or
                # user-installed bash on PATH should win over a
                # possibly-old ``/bin/bash``, and on systems where
                # bash isn't at ``/bin`` at all the exported script
