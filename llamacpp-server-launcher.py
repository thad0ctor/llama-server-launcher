...
# Import system utilities from the new module
from system_utils import (
    # Global variables
    torch, TORCH_AVAILABLE, Llama, LLAMA_CPP_PYTHON_AVAILABLE, 
    psutil, PSUTIL_AVAILABLE, MISSING_DEPS,
    # System info functions
    get_gpu_info_static, get_ram_info_static, get_cpu_info_static,
    # GGUF analysis functions
    analyze_gguf_with_llamacpp_tools, calculate_total_gguf_size,
    parse_gguf_header_simple, analyze_gguf_model_static, run_gguf_analysis
)
...
class LlamaCppLauncher:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.n_predict.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        # Bind trace to ignore_eos to update default config name if needed
        self.ignore_eos.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        # Bind trace to other variables that affect the default config name
        self.cache_type_k.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        self.threads.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        self.threads_batch.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        self.batch_size.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        self.ubatch_size.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        self.ctx_size.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        self.seed.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        self.temperature.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        self.min_p.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        # GPU related updates are handled by _on_gpu_selection_changed (which calls _update_recommendations and then generate_default_config_name)
        self.flash_attn.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        self.tensor_split.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        self.main_gpu.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        self.no_mmap.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())

        self.prio.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        self.mlock.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
        self.no_kv_offload.trace_add("write", lambda *args: self.config_mgr.update_default_config_name_if_needed())
    ...
    def _save_configuration(self):
        ...
        current_cfg = self.config_mgr.current_cfg()
        self.saved_configs[name] = current_cfg
        self._save_configs()
        self.config_mgr.update_config_listbox()
    ...
    def _create_widgets(self):
        ...
        ttk.Button(bar, text="Save SH Script", command=self.save_sh_script).pack(side="left", padx=5)
    ...
    def _setup_config_tab(self, parent):
        ...
        ttk.Button(btn_frame, text="Load Configuration", command=self.config_mgr.load_configuration).pack(side="left", padx=5)
        ttk.Button(btn_frame, text="Delete Configuration", command=self.config_mgr.delete_configuration).pack(side="left", padx=5)
        
        # Import/Export buttons
        import_export_frame = ttk.Frame(inner); import_export_frame.pack(fill="x", padx=10, pady=5)
        ttk.Button(import_export_frame, text="Export Selected", command=self.config_mgr.export_configurations).pack(side="left", padx=5)
        ttk.Button(import_export_frame, text="Import from File", command=self.config_mgr.import_configurations).pack(side="left", padx=5)
        ...
        self.config_listbox.bind("<<ListboxSelect>>", lambda e: self.config_mgr.on_config_selected())
        ...
        if not model_names:
            ...
            if selected_idx != -1:
                ...
                self.root.after(50, self._select_model_in_listbox, selected_idx)
                 ...
                 self.config_mgr.generate_default_config_name() # Generate default name for no model state
    ...
    def _on_model_selected(self, event=None):
        ...
        if selected_name in self.found_models:
            ...
            if LLAMA_CPP_PYTHON_AVAILABLE:
                 ...
                 self.analysis_thread = Thread(target=run_gguf_analysis, args=(full_path_str, self.llama_cpp_dir.get().strip() if hasattr(self, 'llama_cpp_dir') else None, lambda result: self.root.after(0, self._update_ui_after_analysis, result), lambda: self.model_path.get()), daemon=True)
                 ...
                 if llama_dir:
                     ...
                     self.config_mgr.generate_default_config_name() # Generate default name even without analysis
                     self._update_manual_model_visibility() # Update manual model section visibility
    ...
    def _force_analyze_model(self):
        ...
        self.analysis_thread = Thread(target=run_gguf_analysis, args=(model_path_str, self.llama_cpp_dir.get().strip() if hasattr(self, 'llama_cpp_dir') else None, lambda result: self.root.after(0, self._update_ui_after_analysis, result), lambda: self.model_path.get()), daemon=True)
        self.analysis_thread.start()
    ...
    def _update_ui_after_analysis(self, analysis_result):
        ...
        self.config_mgr.generate_default_config_name()
...
# ═════════════════════════════════════════════════════════════════
    ...
    def _on_gpu_selection_changed(self, index):
         ...
         self.app_settings["selected_gpus"] = [i for i, v in enumerate(self.gpu_vars) if v.get()]
         self._update_recommendations()
         self.config_mgr.generate_default_config_name() # Update default config name as it depends on selected GPUs
    ...
    # ═════════════════════════════════════════════════════════════════
    #  Configuration methods have been moved to config.py
    # ═════════════════════════════════════════════════════════════════
    # The following methods are now handled by self.config_mgr:
    # - _generate_default_config_name() -> config_mgr.generate_default_config_name()
    # - _update_default_config_name_if_needed() -> config_mgr.update_default_config_name_if_needed()
    # - _current_cfg() -> config_mgr.current_cfg()
    # - _load_configuration() -> config_mgr.load_configuration()
    # - _delete_configuration() -> config_mgr.delete_configuration()
    # - _on_config_selected() -> config_mgr.on_config_selected()
    # - _update_config_listbox() -> config_mgr.update_config_listbox()
    # - _export_configurations() -> config_mgr.export_configurations()
    # - _import_configurations() -> config_mgr.import_configurations()
