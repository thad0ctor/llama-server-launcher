...
def run_gguf_analysis(model_path_str, llama_cpp_dir=None, callback=None, model_path_getter=None):
    ...
    if model_path_getter is None or model_path_getter() == model_path_str:
         # Try llama.cpp tools first, fall back to llama-cpp-python if available
         
         if llama_cpp_dir:
             print(f"DEBUG: Trying llama.cpp tools from: {llama_cpp_dir}", file=sys.stderr)
             analysis_result = analyze_gguf_with_llamacpp_tools(model_path_str, llama_cpp_dir)
